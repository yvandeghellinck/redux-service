...
const ServiceClass = class {
	...
	launchRequest(dispatch) {
		return new Promise((resolve, reject) => {
			const options = this.generateAjaxOption(dispatch);
			const defaultSuccess = options.success;
			const defaultError = options.error;
	...
	generateAjaxOption(dispatch) {
		const ajaxOptions = this.defaultAjaxOptions(dispatch);
		...
		if (this.options.success) {
			const defaultsSuccess = ajaxOptions.success;
		...
		if (this.options.error) {
			const defaultError = ajaxOptions.error;
			ajaxOptions.error = (res) => {
				defaultError(res);
